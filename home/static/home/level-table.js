// osrs exp table in dictionary format
// levels 1-99 with their associated exp
var expTable = {1:0,2:83, 3:174, 4:276, 5:388, 6:512, 7:650, 8:801, 9:969, 10:1154,
                ...
                71:814445, 72:899257, 73:992895, 74:1096278, 75:1210427, 76:1336443, 77:1475581, 78:1629200, 79:1798808, 80:1986068,
...
// takes an int from 1-99 that represents
// level and returns the exp value where that
// level is hit, ex: 92 -> 6517253
function levelToExp(x) {
    return expTable[x];
}

// takes an int as input, returns
// the level associated with the input
// when the input is taken as the exp
// ex: [174, 275] -> 3
function expToLevel(x, lowerBound, upperBound) {
    ...
    else if (x >= expTable[Math.ceil((upperBound-lowerBound)/2)+lowerBound]) {
        return expToLevel(x, Math.ceil((upperBound-lowerBound)/2)+lowerBound, upperBound)
    }

    else if (x <= expTable[Math.ceil((upperBound-lowerBound)/2)+lowerBound]) {
        return expToLevel(x, lowerBound, Math.ceil((upperBound-lowerBound)/2)+lowerBound - 1)
    }
}